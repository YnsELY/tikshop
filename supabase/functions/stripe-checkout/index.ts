...
Deno.serve(async (req) => {
  try {
    ...
    const { price_id, success_url, cancel_url, mode, quantity = 1, shipping_rate_id, metadata } = requestBody;
...
type ExpectedTypeWithNumber = 'string' | 'number' | { values: string[] };
type Expectations<T> = { [K in keyof T]: ExpectedType };
type ExpectationsWithNumber<T> = { [K in keyof T]: ExpectedTypeWithNumber };

function validateParameters<T extends Record<string, any>>(values: T, expected: ExpectationsWithNumber<T>): string | undefined {
