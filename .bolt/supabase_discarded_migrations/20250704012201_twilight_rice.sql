/*
  # Migration: Créer la table des variantes de produits avec données de test

  1. Nouvelles tables
    - `product_variants`
      - `id` (uuid, primary key)
      - `product_id` (uuid, foreign key vers products)
      - `color` (text, couleur de la variante)
      - `size` (text, taille de la variante)
      - `stock` (integer, stock disponible)
      - `sku` (text, référence unique de la variante)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Modifications
    - Suppression de la colonne `stock` de la table `products` (si elle existe)

  3. Sécurité
    - Activation de RLS sur `product_variants`
    - Politique de lecture publique pour les variantes
    - Politique de gestion pour les vendeurs

  4. Index
    - Index sur `product_id`, `color`, `size`, `sku` pour optimiser les performances

  5. Données de test
    - Création de profils vendeurs
    - Création de produits avec variantes complètes
*/
...
-- Créer des profils vendeurs et des produits de test
DO $$
DECLARE
  ...
  chemise_id uuid;
BEGIN
  -- Créer les profils vendeurs d'abord
  INSERT INTO profiles (id, email, first_name, last_name, phone, country) VALUES
  (seller1_id, 'seller1@tikshop.com', 'Marie', 'Dupont', '+33123456789', 'France'),
  (seller2_id, 'seller2@tikshop.com', 'Pierre', 'Martin', '+33123456790', 'France'),
  (seller3_id, 'seller3@tikshop.com', 'Sophie', 'Bernard', '+33123456791', 'France'),
  (seller4_id, 'seller4@tikshop.com', 'Lucas', 'Petit', '+33123456792', 'France'),
  (seller5_id, 'seller5@tikshop.com', 'Emma', 'Durand', '+33123456793', 'France')
