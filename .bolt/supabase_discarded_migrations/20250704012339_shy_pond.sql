/*
  ...
  3. Security
    ...
    - Add policies for variant creation (admin only)

  4. Sample Data
    - Create sample products with variants (colors, sizes, stock)
    - Use existing seller profiles or create minimal test data
*/
...
-- Créer des produits de test et leurs variantes
DO $$
DECLARE
  -- Utiliser des IDs fixes pour éviter les conflits
  seller1_id uuid := '11111111-1111-1111-1111-111111111111';
  seller2_id uuid := '22222222-2222-2222-2222-222222222222';
  seller3_id uuid := '33333333-3333-3333-3333-333333333333';
  seller4_id uuid := '44444444-4444-4444-4444-444444444444';
  seller5_id uuid := '55555555-5555-5555-5555-555555555555';
  ...
  chemise_id uuid;
BEGIN
  -- Vérifier si des profils existent déjà, sinon créer des profils minimaux
  -- (sans contrainte de clé étrangère vers users)
  INSERT INTO profiles (id, email, first_name, last_name, phone, country) 
  SELECT seller1_id, 'seller1@tikshop.com', 'Marie', 'Dupont', '+33123456789', 'France'
  WHERE NOT EXISTS (SELECT 1 FROM profiles WHERE id = seller1_id);
  
  INSERT INTO profiles (id, email, first_name, last_name, phone, country) 
  SELECT seller2_id, 'seller2@tikshop.com', 'Pierre', 'Martin', '+33123456790', 'France'
  WHERE NOT EXISTS (SELECT 1 FROM profiles WHERE id = seller2_id);
  
  INSERT INTO profiles (id, email, first_name, last_name, phone, country) 
  SELECT seller3_id, 'seller3@tikshop.com', 'Sophie', 'Bernard', '+33123456791', 'France'
  WHERE NOT EXISTS (SELECT 1 FROM profiles WHERE id = seller3_id);
  
  INSERT INTO profiles (id, email, first_name, last_name, phone, country) 
  SELECT seller4_id, 'seller4@tikshop.com', 'Lucas', 'Petit', '+33123456792', 'France'
  WHERE NOT EXISTS (SELECT 1 FROM profiles WHERE id = seller4_id);
  
  INSERT INTO profiles (id, email, first_name, last_name, phone, country) 
  SELECT seller5_id, 'seller5@tikshop.com', 'Emma', 'Durand', '+33123456793', 'France'
  WHERE NOT EXISTS (SELECT 1 FROM profiles WHERE id = seller5_id);
