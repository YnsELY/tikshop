...
interface ProductHourlySalesChartProps {
  productId: string;
  productName: string;
}
...
export const ProductHourlySalesChart: React.FC<ProductHourlySalesChartProps> = ({ 
  productId, 
  productName 
}) => {
  ...
  const loadProductHourlyData = async (date: string) => {
    if (!user?.profile?.is_admin || !productId) return;
    ...
    try {
      ...
      const productStats = await getProductStats(productId, startDate, endDate);
      
      if (productStats && productStats.hourly_sales) {
        setHourlyData(productStats.hourly_sales);
      } else {
        // Si pas de données, créer un tableau vide avec toutes les heures
      ...
      console.error('Error loading product hourly data:', err);
  ...
  useEffect(() => {
    loadProductHourlyData(selectedDate);
  }, [selectedDate, productId, user]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold flex items-center">
              ...
              Ventes par heure - {productName}
            </h3>
            <p className="text-sm text-gray-600">
              Répartition des ventes de ce produit sur 24 heures
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
            <div className="flex items-center justify-center mb-2">
              ...
              <span className="text-sm font-medium text-blue-800">Articles vendus</span>
            </div>
        ...
        {!isLoading && !error && hourlyData.every(item => item.sales === 0) && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">
              Aucune vente de "{productName}" le {format(new Date(selectedDate), 'dd MMMM yyyy', { locale: fr })}
            </p>
          </div>
        )}
      </div>
